...
CREATE TABLE fe_users (
    ...
    tx_maiaccount_member_ref varchar(64) DEFAULT '' NOT NULL,

    -- Comma-separated list of interest identifiers
    tx_maiaccount_interests text,

    -- Newsletter opt-in flag
    tx_maiaccount_newsletter_optin tinyint(1) unsigned DEFAULT 0 NOT NULL,

    -- Timestamp when newsletter opt-in was confirmed
    tx_maiaccount_newsletter_optin_date int(11) unsigned DEFAULT 0 NOT NULL,

    -- Event reminder opt-in flag
    tx_maiaccount_reminders_optin tinyint(1) unsigned DEFAULT 0 NOT NULL,

    -- Email confirmation token for registration
    tx_maiaccount_confirmation_token varchar(128) DEFAULT '' NOT NULL,

    -- Timestamp when confirmation token expires
    tx_maiaccount_confirmation_token_expires int(11) unsigned DEFAULT 0 NOT NULL,

    -- Whether email has been confirmed
    tx_maiaccount_email_confirmed tinyint(1) unsigned DEFAULT 0 NOT NULL,

    -- Password reset token
    tx_maiaccount_password_reset_token varchar(128) DEFAULT '' NOT NULL,

    -- Timestamp when password reset token expires
    tx_maiaccount_password_reset_token_expires int(11) unsigned DEFAULT 0 NOT NULL,

    -- MFA/TOTP secret (encrypted)
    tx_maiaccount_mfa_secret varchar(256) DEFAULT '' NOT NULL,

    -- Whether MFA is enabled
    tx_maiaccount_mfa_enabled tinyint(1) unsigned DEFAULT 0 NOT NULL,

    -- JSON-encoded array of hashed backup codes
    tx_maiaccount_mfa_backup_codes text
);
...
CREATE TABLE tx_maiaccount_reminder_queue (
